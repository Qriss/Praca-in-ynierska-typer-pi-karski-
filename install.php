...
/*---------------wstawianie druzyn do tabeli access-----------------------------------*/
/*
$kol = array();
$kol[] = 'Gosc';
$kol[] = 'Administrator';
...
*/
/*---------------stworzenie konta dla administratora-----------------------------------*/
/*
$login = "Admin";
$pass  = "qwerty";
$email = "qriss@hotmail.com";

$pass = crypt($pass);

try{
    $stmt= $pdo -> prepare('INSERT INTO `uzytkownik` (`login`, `haslo`, `email`)
                               VALUES(:login, :haslo, :email)');
    $stmt -> bindValue(':login', $login, PDO::PARAM_STR);
    $stmt -> bindValue(':haslo', $pass, PDO::PARAM_STR);
    $stmt -> bindValue(':email', $email, PDO::PARAM_STR);
    $result = $stmt->execute();
    $stmt->closeCursor();

    echo nl2br("Wpisano wartosci w tabele uzytkownik!\n");
}
catch(\PDOException $e){
...
*/
/*---------------powiazanie uzytkownika z kontem admnistratora-----------------------------------*/

/*
try{
    $stmt= $pdo -> prepare('INSERT INTO `accessUser` (`idUser`, `idAccess`)
                               VALUES(:idUser, :idAccess)');
    $stmt -> bindValue(':idUser', 1, PDO::PARAM_INT);
    $stmt -> bindValue(':idAccess', 2, PDO::PARAM_INT);
    $result = $stmt->execute();
    $stmt->closeCursor();
    $error =$pdo->errorInfo();
    echo "błąd: ".$error[2];
    echo nl2br("Wpisano wartosci w tabele accesuser!\n");
}
catch(\PDOException $e){
    $error =$pdo->errorInfo();
    echo "błąd: ".$error[2];
