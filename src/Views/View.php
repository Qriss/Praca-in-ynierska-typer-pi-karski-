...
abstract class View {
    ...
    public function  __construct() {
        ...
        if(\Tools\Access::islogin() === true)
            $this->set('login',true);
            $this->set('name', \Tools\Session::get('user'));
            $model = $this->getModel('Uzytkownik');
            $item = $model->getOne(\Tools\Session::get('user'));
            $this->set('Uzytkownik', $item);
            $this->set('access', \Tools\Session::get('access'));
    ...
    public function render($name) {
        ...
        try {
            if(is_file($path)) {
                ...
                throw new \Exception('Nie mozna zalaczyc szablonu '.$name.' z: '.$path);
            }
        }
        catch(Exception $e) {
